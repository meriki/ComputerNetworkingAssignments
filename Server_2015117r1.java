...
class App extends Thread{
    ...
    void writeToClient(int x, String msg){
        if(this.id!=x){
            ...
            try{
                if(Server_2015117r1.connections.get(x).isRunning == true){
                    output = new DataOutputStream(Server_2015117r1.connections.get(x).connection.getOutputStream());
                    output.writeUTF(msg);
                    output.flush();
                }                
            }
            catch(IOException e2){
                System.out.println("Client " + Server_2015117r1.connections.get(x).id + "disconnected ");
                Server_2015117r1.connections.get(x).isRunning = false;
            }  
        ...
        else if(Server_2015117r1.connections.get(x).isRunning == true){
            giveErrorMessage(x + " is self");
        }      
    }

    void writeToAllClients(String msg){
        for(App connect: Server_2015117r1.connections){
